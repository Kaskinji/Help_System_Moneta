...
def extract_relevant_part(text: str, query: str, min_similarity: float = 0.7) -> str:
    ...
    # Кодируем только текст предложений (без разделителей)
    sentence_embeddings = model.encode([re.sub(r'[.!?]\s*$', '', s) for s in sentences])

    # Расчет схожести
    similarities = cosine_similarity([query_embedding], sentence_embeddings)[0]

    # Находим индекс первого релевантного предложения
    ...
    if first_relevant_idx == -1:
        return sentences[np.argmax(similarities)].strip()  # Возвращаем самое похожее

    # Объединяем предложения с сохранением оригинальной пунктуации
    relevant_part = ''.join(sentences[first_relevant_idx:]).strip()

    # Удаляем лишние точки в середине предложений
...
def search_precise_answer(user_query: str, limit: int = 1):
    """Поиск ответа с новой логикой извлечения"""
    try:
